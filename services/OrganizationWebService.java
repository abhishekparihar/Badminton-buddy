...
public class OrganizationWebService implements WebServiceIface {
    ...
    public static List<OrganizationResult> getAll(Double[] locationParams, int page) {
        ...
        LogUtils.LOGV("CHECKINFORGOOD", "Allcause response: " + response);

        try {
            java.lang.reflect.Type typeOfListOfAllOrganizationResult = (new TypeToken<List<OrganizationResult>>() {
            }).getType();
            res = new Gson().fromJson(response, (java.lang.reflect.Type) typeOfListOfAllOrganizationResult);
        } catch (Exception e) {
        	LogUtils.LOGE("CHECKINFORGOOD", "AllOrganizationResult Error: " + e.getMessage());
        }
    ...
    public static List<OrganizationResult> getMy(Double[] locationParams) {
        ...
        if ( response == null ) {
        	LogUtils.LOGV("CHECKINFORGOOD", "Null response from organization web service");
        } else {
        	LogUtils.LOGE("CHECKINFORGOOD", "All causes result for lat " + locationParams[0] + " lon " + locationParams[1] + ": " + response.toString());
        }

        try {
            java.lang.reflect.Type typeOfListOfResult = (new TypeToken<List<OrganizationResult>>() {
            }).getType();
            res = new Gson().fromJson(response, (java.lang.reflect.Type) typeOfListOfResult);

            for ( OrganizationResult cause : res ) {
            	LogUtils.LOGE("CHECKINFORGOOD", "CAUSE LIST " + cause.getOrganization().getName());
            }
        } catch (Exception e) {
        	LogUtils.LOGE("CHECKINFORGOOD", "MyOrganizationResult Error: " + e.getMessage());
        }
    ...
    public static Boolean[] supportOrg(Integer orgId, Boolean isSupported) {
        ...
        String response = webService.webPost("", params);
        LogUtils.LOGV("CHEcKINFOGOOD", "supportOrg response: " + response);
        ...
        try {
        	...
        	LogUtils.LOGE("CHECKINFORGOOD", "supportOrg Error: " + e.getMessage());
        }
    ...
    public static List<OrganizationResult> search(CharSequence searchText) {
        ...
        if ( response == null ) {
        	LogUtils.LOGV("CHECKINFORGOOD", "Null response from organization search web service");
        } else {
        	LogUtils.LOGE("CHECKINFORGOOD", "Organization search response: " + response.toString());
        }
        ...
        try {
        	...
        	LogUtils.LOGE("CHECKINFORGOOD", "Cause search error: " + e.getMessage());
        }
