...
public class SocialSharingWebService implements WebServiceIface {
    ...
    public static boolean doSharePreferences(String[] allparams) {
        WebService webService = new WebService(BASE_URL + "/mobile_user/sharing_preferences.json");
    ...
    public static boolean doUpdatePreferences(String[] allparams) {
        WebService webService = new WebService(BASE_URL + "/mobile_user/update_sharing_preferences.json");
    ...
    public static boolean getSharedPreferences(String[] allparams) {
        ...
        WebService webService = new WebService(BASE_URL + "/mobile_user/get_shared_preferences.json");
        ...
        if ( response != null ) {
            ...
            try {
                jsonResult = new JSONObject(response);
                SocialSharingResult res = new SocialSharingResult(jsonResult.getString("facebook_social_preference"), jsonResult.getString("twitter_social_preference"));

                writeTosharedPreferences(res);
                return true;
            } catch (JSONException e) {
            	LogUtils.LOGE("FaceBook Sharing "," in Heads "+e.toString());
                e.printStackTrace();
                return false;
            }
        }
    ...
    private static void writeTosharedPreferences(SocialSharingResult socialObject) {
        ...
        if ( socialObject.facebookObject.getExist() ) {
            if ( socialObject.facebookObject.getAccessToken() != null )
                appStatus.saveSharedStringValue(appStatus.FACEBOOK_TOKEN, socialObject.facebookObject.getAccessToken());
        ...
        if ( socialObject.twitterObject.getExist() ) {
            if ( (socialObject.twitterObject.getAccessToken() != null) && (socialObject.twitterObject.getAccessSecret() != null) ) {
                appStatus.saveSharedStringValue(appStatus.TWITTER_TOKEN, socialObject.twitterObject.getAccessToken());
                appStatus.saveSharedStringValue(appStatus.TWITTER_SECRET, socialObject.twitterObject.getAccessSecret());
            }
    ...
    private static void updateExpiredToken(Boolean bTokenExpired, String fbAccessToken) {
        ...
        if ( bTokenExpired ) {
            ...
            args[2] = Boolean.toString(CheckinLibraryActivity.appStatus.getSharedBoolValue(CheckinLibraryActivity.appStatus.FACEBOOK_ON));
